# T????K Tourism Forecasting Project

## 1. Project Overview

This project develops a quarterly forecasting model for domestic tourism in Turkey using official T????K data. The selected variable is the total number of trips made by domestic visitors, observed quarterly from 2009 Q1 to 2025 Q4. Ten alternative forecasting methods are applied, compared using standard accuracy measures, and the superior method is selected to forecast 2026 Q1.

## 2. Data Source and T????K Connection

Data were accessed directly from the T????K Data Portal using the `tuikr` R package combined with `httr::GET()` to download the istab file. No data was manually downloaded, edited, or saved outside of R.

- **T????K dataset name:** Number of Trips of Domestic Visitors by Type of Accommodation and Quarter
- **T????K theme/category:** Tourism (Theme ID: 14)
- **T????K table name:** Number of Trips of Domestic Visitors by Type of Accommodation and Quarter
...
- **Na??ve Forecasting** ??? applicable, used as baseline
...
- **Date of data access:** 2026-06-02
- **Latest available T????K observation:** 2025 Q4
- **Forecast target period:** 2026 Q1
...
- Data revisions by T????K could affect reproducibility if re-run at a later date
...
No manual data preparation is required. Data is fetched live from T????K at knit time.
